# Deployment Guide — Valheim ServerGuard 2.0
...
- The compiled DLL from this repo: `Thunderstore files/Valheim-ServerGuard/Valheim-ServerGuard.dll`. **The same file goes on the server and on every player's PC** — since 2.0 there is one mod, and it detects which side it is on at load.
...
### B1. Install ServerGuard on the client
...
4. Install `Valheim_ServerGuard` from Thunderstore, or drop `Valheim-ServerGuard.dll` into `BepInEx/plugins/`.

> Upgrading from 1.x: remove the old `Valheim_ServerGuard_Client` package first. It must not run next to 2.0.

If installing manually:

```
<your Valheim install>\BepInEx\plugins\Valheim-ServerGuard.dll
```
...
```
[Info: Valheim ServerGuard] [ServerGuard] v2.0.0 starting as CLIENT (mode=auto, headless=False).
```
...
```yaml
# ServerGuard - allowed_mods snippet generated by ServerGuard.Client v2.0.0
# Generated: 2026-05-10 09:42:01Z   Mods on this client: 29
# ...

required_mods:
  - com.taeguk.valheim.serverguard|<hash>    # Valheim ServerGuard v2.0.0
...
```
[Info: Valheim ServerGuard] [ServerGuard.Client] Registered manifest request handler on server peer.
[Info: Valheim ServerGuard] [ServerGuard.Client] Sent manifest (1700 bytes, 29 mods).
```
...
1. The file `Valheim-ServerGuard.dll` (or the `Valheim_ServerGuard` Thunderstore package).
...
Either the player isn't running `Valheim-ServerGuard.dll`, or BepInEx isn't loading on their client. Have them check their own `BepInEx/LogOutput.log` for `Loading [Valheim ServerGuard 2.0.0]` followed by `starting as CLIENT`. If absent, BepInEx isn't installed correctly on their side.
...
| Client plugin DLL (same file as the server) | `<player profile>\BepInEx\plugins\Valheim-ServerGuard.dll` |
