...
Players join the server normally. Every log line and Discord message identifies the player by **character name + Steam ID** (the name comes from `registrations.yaml`; brand-new players show as `NewPlayer` until they pick a character). The server log (in `BepInEx/LogOutput.log`) shows lines like:
