...
- `ServerGuardPlugin.cs` — `public const string VERSION = "2.0.1";` (the only literal in code; `ServerPlugin`'s log/config strings and `ClientPlugin.VERSION` read it)
- `Valheim-ServerGuard.csproj` — `<Version>2.0.1</Version>`
- `Thunderstore files/Valheim-ServerGuard/manifest.json` — `"version_number": "2.0.1"`
