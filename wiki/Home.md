...
A dedicated-server mod for Valheim that locks the server to a specific modpack, provides anti-cheat enforcement, Discord logging, admin tools, and forensic logs. Server-side mod paired with a required companion plugin on every client.
...
- Not a server-side framework — it's a single mod with a companion.
