...
This guide walks through building `Valheim-ServerGuard.dll` from source on Windows. One DLL is produced; it is installed unchanged on both the dedicated server and every player's game.
...
Confirm BepInEx itself is installed and running — check `BepInEx\LogOutput.log` for a line like `[Info: ServerGuard] v2.0.0 starting as SERVER ...`.
